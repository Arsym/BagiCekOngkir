...
import com.bagicode.cekongkir.model.subdistrict.Result;
...
public class SubdistrictAdapter extends BaseAdapter {
     ...
     */
    public SubdistrictAdapter(Activity activity, List<Result> movieItems) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        tv_detail.setText(m.getSubdistrictId());
    ...
    public void filter(String charText)
    {
        ...
        if (charText.length() == 0) {
            ...
            for (Result lok : listlokasiasli) {
                if (lok.getCity().toLowerCase().contains(charText)) {
                    movieItems.add(lok);
                } else {
                }
